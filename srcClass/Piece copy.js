...
export class Piece extends THREE.Mesh {
	constructor ( vertices, color, camera, tangram, sizeCircle = 0.09, sizeEdge = 4 ) {
	...
	getClosestVertex ( point ) {
		...
		for ( let i = 0; i < this.vertices.length; i += 3 ) {
			const vertex = new THREE.Vector3( this.vertices[i], this.vertices[i + 1], 0 ).applyMatrix4( this.matrixWorld );
		...
		this.updatePivotRotatePositionIndex( closestIndex )
		// return closestIndex;
	}
	...
	rotateAroundWorldPoint ( angle ) {
		const point = this.pivotRotate.position.clone().applyMatrix4( this.matrixWorld );
